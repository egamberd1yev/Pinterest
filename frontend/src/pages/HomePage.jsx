...
useEffect(() => {
  const fetchPins = async () => {
    ...
    try {
      const res = query
        ? await api.get(`/image?search=${encodeURIComponent(query)}`)
        : await api.get('/image')
      setPins(res.data)
    } catch (err) {
      ...
      {filteredPins.length === 0 ? (
        <div className="flex flex-col items-center justify-center mt-20 gap-3">
          ...
          <p className="text-gray-400 text-lg">
            {search ? `"${search}" bo'yicha hech narsa topilmadi` : "Hali pinlar yo'q"}
          </p>
          <button
        ...
        <div className="p-4" style={{ columnCount: 4, columnGap: '12px' }}>
